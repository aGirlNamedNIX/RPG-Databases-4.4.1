...
func start():
	...
	for i in range(json_dictionary.size()):
		var chara_data = json_dictionary["chara"+str(i)]
...
func refresh_data(id: int):
	var json_dictionary: Dictionary = get_parent().get_parent().call("read_data", "Character")
	var chara_data: Dictionary = json_dictionary["chara"+str(id)]
	...
	for equip in e_type_dictionary.keys():
		...
		var type_id: int = e_type_dictionary[equip]
		equip_id_array.append(e_type_dictionary[equip])
		match kind:
			"w":
				var w_id = equip.erase(0, 1)
				equip_name = "W: " + w_type_dictionary[str(type_id)]
				$EquipLabel/EquipContainer/EquipContainer/EquipList.add_item(equip_name)
			"a":
				var a_id = equip.erase(0, 1)
				equip_name = "A: " + a_type_dictionary[str(type_id)]
				$EquipLabel/EquipContainer/EquipContainer/EquipList.add_item(equip_name)
	...
	for equip in equip_slots_dictionary.keys():
		...
		var kind: String = equip[0]
		var kind_id = str(equip)
		kind_id = kind_id.erase(0, 1)
		match kind:
			"w":
				var w_id: int = -1
				...
				if w_id >= 0:
					var weapon_data: Dictionary = weapon_list["weapon"+str(w_id)]
			...
			"a":
				...
				if a_id >= 0:
					var armor_data: Dictionary = armor_list["armor"+str(a_id)]
...
func _on_AddButton_pressed() -> void:
	...
	json_dictionary["chara"+str(id)] = character_data
...
func _on_RemoveCharacterButton_pressed():
	...
	if json_dictionary.keys().size() > 1:
		...
		while chara < (json_dictionary.keys().size() - 1):
			json_dictionary["chara"+str(chara)] = json_dictionary["chara"+str(chara+1)]
			chara += 1
		json_dictionary.erase("chara"+str(chara))
		get_parent().get_parent().call("store_data", "Character", json_dictionary)
...
func save_character_data() -> void:
	var json_dictionary: Dictionary = get_parent().get_parent().call("read_data", "Character")
	var chara_data: Dictionary = json_dictionary["chara"+str(character_selected)]
	...
	for i in range(equip_items):
		...
		match kind:
			"W":
				equip_type_data["w"+str(i)] = equip_id_array[i]
			"A":
				equip_type_data["a"+str(i)] = equip_id_array[i]
...
func _on_AddEquipTypeButton_pressed() -> void:
	...
	for i in w_type_data.size():
		if i > $EquipLabel/AddEquip/EquipLabel/EquipButton.get_item_count() - 1:
			$EquipLabel/AddEquip/EquipLabel/EquipButton.add_item(w_type_data[str(i)])
		else:
			$EquipLabel/AddEquip/EquipLabel/EquipButton.set_item_text(i, w_type_data[str(i)])
	...
	match index:
		...
		1:
			...
			for i in range(a_type_data.size()):
				if i > $EquipLabel/AddEquip/EquipLabel/EquipButton.get_item_count() - 1:
					$EquipLabel/AddEquip/EquipLabel/EquipButton.add_item(a_type_data[str(i)])
				else:
					$EquipLabel/AddEquip/EquipLabel/EquipButton.set_item_text(i, a_type_data[str(i)])
...
func _on_EquipList_item_activated(index: int) -> void:
	...
	elif slots_data.has("a"+str(index)):
		var armor_list: Dictionary = get_parent().get_parent().call("read_data", "Armor")
